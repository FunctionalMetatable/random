...
void(async () => { // Make sure there is no result so the page isn't overwritten
    ...
    (menuItems => {
        ...
        d.setAttribute("data-autofill-menu", "1"); // let the CSS see it exists without clashing with on-page styles
        let s = document.createElement("style"); // make a style tag
        s.textContent = "[data-autofill-menubutton]:not(:root) { outline: none; margin: 0px; border: none; " + // add the CSS
                ...
                "0s !important; }[data-autofill-menubutton]:not(:root):hover, [data-autofill-menubutt" +
                "on]:not(:root):focus { border: none; border-radius: 0px; background: rgb(200, 20" +
                "0, 201); color: inherit; font: inherit; }[data-autofill-menubutton]:not(:root):dis" +
                "abled { color: rgb(114, 119, 124); background: transparent; cursor: default; poi" +
                "nter-events: none; }[data-autofill-menu]:not(:root) { line-height: 1.45; border: 1" +
                ...
                "3px 3px 2px -2px; }[data-autofill-blockbg] { position: fixed; top: 0px; left: 0px;" +
                " width: 100%; height: 100%; z-index: 2147483647; }";
        let c = _ => [...document.querySelectorAll("[data-autofill-menu],[data-autofill-blockbg]")].map(e => e.remove()); // this is a function that closes the menu
        d.append(...menuItems.map(e => { // this part turns the menu items into button tags
            ...
            b.setAttribute("data-autofill-menubutton", "1"); // let the CSS see it exists without clashing with on-page styles
        ...
        b.setAttribute("data-autofill-blockbg", "1"); // let the CSS see it exists without clashing with on-page styles
        ...
        : [ // if we aren't on a forum topic, tell the user.
            {
                text: "Please use Autofill on the forums!"
            }
        ]);
})()
